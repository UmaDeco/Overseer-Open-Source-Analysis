...
def master_rows(master_data, name):
    return (
        master_data.get("tables", {}).get(name)
        or master_data.get("text", {}).get(name)
        or []
    )
...
def build_race_context(master_data):
    ...
    course_sets = {
        ...
        for row in master_rows(master_data, "race_course_set")
    }
    instances = {
        int(row.get("id") or 0): row
...
def synthesize_public_race_data(base_dir, race_context, existing_meta=None):
    ...
    def sort_key(item):
        year_offset = next(
            offset
            for offset, label in YEAR_LABELS.items()
            if item["date"].startswith(label)
        )
...
def synthesize_race_map(base_dir, master_data):
    ...
    if programs:
        public_result = synthesize_public_race_data(
            base_dir, race_context, existing.get("meta") or {}
        )
        current_public = read_json(
            Path(base_dir) / "public" / "assets" / "data" / "uma_race_data.json", {}
        )
        generated_by_key = {
            (row["name"], row["date"]): row for row in current_public.get("races", [])
        }
...
def synthesize_card_aptitudes(base_dir, master_data):
    # Base aptitude grades per trainee card (1=G .. 8=S), used by the Setup
    # "Aptitudes" panel to preview letters with parent factors applied.
    # card_rarity_data repeats the same proper_* values on every rarity row of
    # a card, so the last row per card_id wins harmlessly.
    data_dir = Path(base_dir) / "data"
    fields = [
        "proper_ground_turf",
        "proper_ground_dirt",
    ...
    for row in master_rows(master_data, "card_rarity_data"):
        ...
        if not card_id:
            continue
        aptitudes[str(card_id)] = [int(row.get(f) or 0) for f in fields]

    if aptitudes:
        write_json(data_dir / "card_aptitudes.json", aptitudes)
    return {"file": "card_aptitudes.json", "rows": len(aptitudes)}


def synthesize_support_hints(base_dir, master_data):
    # Hint skills each support card can teach during training (the Setup
    # INHERITANCE panel's "Support hints" row). single_mode_hint_gain rows
    # with hint_gain_type 0 carry a skill id in hint_value_1; ordered by
    # hint_group so the list matches the in-game hint order.
    data_dir = Path(base_dir) / "data"
    skill_names = text_map(master_rows(master_data, "cat_47_text"))
    hints = {}
    rows = sorted(
        master_rows(master_data, "single_mode_hint_gain"),
        key=lambda r: (int(r.get("support_card_id") or 0), int(r.get("hint_group") or 0)),
    )
    for row in rows:
        if int(row.get("hint_gain_type") or 0) != 0:
            continue
        card_id = int(row.get("support_card_id") or 0)
        skill_id = int(row.get("hint_value_1") or 0)
        if not card_id or not skill_id:
            continue
        name = skill_names.get(skill_id, str(skill_id))
        bucket = hints.setdefault(str(card_id), [])
        if name not in bucket:
            bucket.append(name)

    if hints:
        write_json(data_dir / "support_hint_skills.json", hints)
    return {"file": "support_hint_skills.json", "rows": len(hints)}
...
def synthesize_legacy_jsons(base_dir, master_data):
    generated = [
        ...
        synthesize_card_aptitudes(base_dir, master_data),
        synthesize_support_hints(base_dir, master_data),
    ]
...
def generate(base_dir, master_mdb_path=None):
    db_path = (
        Path(master_mdb_path).expanduser()
        if master_mdb_path
    ...
    with sqlite3.connect(str(db_path)) as conn:
        ...
        for table in DUMP_JSON_TABLES:
            if table in existing_tables:
                try:
                    rows = dump_table(cursor, table)
                    write_json(data_dir / f"{table}.json", rows)
                    extracted.append({"table": f"{table}.json", "rows": len(rows)})
                except Exception as e:
                    log.warning(f"{table}.json extraction failed: {e}")
        ...
        if "text_data" in existing_tables:
            ...
            try:
                cursor.execute(
                    "SELECT `index`, text FROM text_data WHERE id = 6 AND category = 6"
                )
                chara_names = {str(row[0]): row[1] for row in cursor.fetchall()}
                write_json(data_dir / "chara_names.json", chara_names)
                extracted.append(
                    {"table": "chara_names.json", "rows": len(chara_names)}
                )
            except Exception as e:
                log.warning(f"chara_names.json extraction failed: {e}")

            try:
                cursor.execute("SELECT `index`, text FROM text_data")
                event_names = {str(row[0]): row[1] for row in cursor.fetchall()}
                write_json(data_dir / "event_names.json", event_names)
                extracted.append(
                    {"table": "event_names.json", "rows": len(event_names)}
                )
            except Exception as e:
                log.warning(f"event_names.json extraction failed: {e}")
